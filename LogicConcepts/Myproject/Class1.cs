public class Ruta
{
    ...
    public void SetPasajeros(int valor)
    {
        if (valor < 0)
            throw new ArgumentException("Los pasajeros no pueden ser negativos");
        pasajeros = valor;
    }
    ...
    {
        return $"Ruta {numeroRuta} con {pasajeros} pasajeros (Base: {ObtenerBase()})";
    }
}
